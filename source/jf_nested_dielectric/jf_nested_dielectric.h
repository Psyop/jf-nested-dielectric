...
typedef struct MediaCache {
    ...
    int   BRDF;    
    float refractDirect;
    float refractIndirect;
    float specDirect;
    float specIndirect;
    float refractRoughnessU;
    float refractRoughnessV;
    float specRoughnessU;
    ...
    MediaCache() {
        ...
        this->BRDF = b_cook_torrance;
        this->refractDirect = 1.0f;
        this->refractIndirect = 1.0f;
        this->specDirect = 1.0f;
        this->specIndirect = 1.0f;
        this->refractRoughnessU = 0.0f;
...
typedef struct InterfaceInfo {
    ...
    bool doBlurryRefraction() 
    {
        return (this->rs->media[this->m1].refractRoughnessU > ZERO_EPSILON 
            ...
            || this->rs->media[this->m2].refractRoughnessV > ZERO_EPSILON);
    }
    ...
    void getRefrRoughness(float &roughnessU, float &roughnessV) 
    {
        ...
        roughnessV = refractiveRoughness(this->rs->media[this->m1].refractRoughnessV, 
                                        this->rs->media[this->m2].refractRoughnessV, this->n1, this->n2 );
    }
    ...
    {       
        ...
        case b_cook_torrance_ray_tangent:
            ...
            blurAnisotropicPoles(&roughnessU, &roughnessV, 
                this->rs->media[this->m_higherPriority].blurAnisotropicPoles, sg->N, tangentSourceVector);
        ...
        float drs_roughnessU, float drs_roughnessV, AtVector customTangentVector, void **btdfA, void **btdfB) 
    {
        ...
        case b_cook_torrance_user_tangent:
            ...
            blurAnisotropicPoles(&dr_roughnessU, &dr_roughnessV, this->rs->media[this->m_higherPriority].blurAnisotropicPoles, ppsg->Nf, customTan);
            blurAnisotropicPoles(&drs_roughnessU, &drs_roughnessV, this->rs->media[this->m_higherPriority].blurAnisotropicPoles, ppsg->Nf, customTan);
...
typedef struct TraceSwitch
{
    ...
    TraceSwitch(InterfaceInfo * iinfo, bool internal_reflections) 
    {
        ...
        this->spec_ind = rs->media[iinfo->m_higherPriority].specIndirect > ZERO_EPSILON;
        this->spec_dir = iinfo->mediaEntrance && rs->media[iinfo->m_higherPriority].specDirect > ZERO_EPSILON;
