...
spectral_wavelength_LUT build_spectral_wavelength_LUT( const spectral_distrib *distrib )
{
	...
	for( int lut_index = 0; lut_index < spectral_lookup_point_num; lut_index++ )
	{
		...
		{			
			const float distance = abs( search_value - subdiv_distrib_iterative.values[subdiv_index] ); // difference between this index and search value
			const float distance_prev = (subdiv_index != 0) ? abs(search_value - subdiv_distrib_iterative.values[subdiv_index - 1]): distance + 10.0f; // difference between previous index and search value
			const float distance_next = (subdiv_index != subdiv_distrib_point_num - 1) ? abs(search_value - subdiv_distrib_iterative.values[subdiv_index + 1]) : distance + 10.0f; // difference between next index and search value
...
AtColor HSVtoRGB(float h, float s, float v )
{
	int i;
	float f, p, q, t;
	if( s == 0 ) {
		return AI_RGB_50GREY;
	}
	h /= 60;			// sector 0 to 5
	i = (int) floor( h );
